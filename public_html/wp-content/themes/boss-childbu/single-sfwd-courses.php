                ...
                <article id="post-<?php the_ID(); ?>" <?php post_class(); ?>>
                    <div class="table">
                        ...
                        <header class="entry-header table-cell">
                           <?php echo badgeos_achievement_points_markup(); ?>
...
<?php else: ?> 
    <?php while ( have_posts() ) : the_post(); ?>
        ...
        <?php
        $style = ( has_post_thumbnail() && boss_get_option( 'boss_cover_blog' ) ) ? 'style="background-image: url(' . get_the_post_thumbnail_url( $post ) . '); background-position: right 40px center;background-size: contain;background-repeat: no-repeat;" data-photo="yes"' : '';
        ?>
...
$completed = $usermeta[$post->ID]['completed'];
$total = $usermeta[$post->ID]['total'];
$percentage = $completed*100/$total;
?>
<header class="profile-banner-on-courses-page" style="background: url('<?php echo $banner_image; ?>') no-repeat right center / cover;">
...
<!-- This section was after header before -->
        ...
        <header class="<?php echo $post->post_name; ?> page-cover table">
            <div class="table-cell page-header" <?php echo $style; ?>>
                <div class="cover-content">
                    <h1 class="post-title main-title">
                    	<?php the_title(); ?>
                    	<small style="font-size: 16px; display: block;"><?php echo "<a href='".get_permalink(get_page_by_path( 'courses-list' ))."' title='Courses'>Courses</a>" .' <span style="font-size: 23px;">›</span> '. get_the_title(); ?></small>
                    </h1>
                    <div class="table">
...
if($total != 0)
{
	echo do_shortcode('[progress_bar size="wide" style="striped" title="Your Progress Through This Course" percent="'.$percentage.'" color="lightgreen"]');
}
?>
...
$('#learndash_course_content').prependTo('.entry-content');

$(document).ready(function(){
	$(".learndash_topic_dots").slideUp(); return false;
});
</script>
