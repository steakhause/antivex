...
$defaultAvatarURL = get_stylesheet_directory_uri() . '/images/mystery.jpg';
?>
        ...
        <?php if(is_user_logged_in()) : ?>
            <div class="userInformation">
                <?php if(current_user_can('fulfillment_role') || current_user_can('administrator')) {
                    echo get_avatar( $userId, 48 ) . ' <span>' . get_user_meta($userId, 'nickname', true) . '</span>';
                } else { ?>
                    <a href="<?php echo get_permalink(1028); ?>"><?php echo get_avatar( $userId, 48 ) . ' <span>' . get_user_meta($userId, 'nickname', true) . '</span>'; ?></a>
                <?php } ?>
                
            	<div class="profile-badges" style="display: none;">
            		<?php
                  $badges = badgeos_get_user_achievements( array( 'user_id' => $userId ) );
               ...
               <a style="display: none;" href="<?php echo wp_logout_url(); ?>" class="fa-file logout-button">Log Out</a>
            </div>
        <?php endif; ?>
    <?php endif; ?>
