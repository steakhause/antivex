...
<?php else: ?>
    <?php while ( have_posts() ) : the_post(); ?>
        ...
        <?php
        $style = 'style="background-image: url(' . get_the_post_thumbnail_url( $course_data->ID ) . '); background-position: right 40px center;background-size: contain;background-repeat: no-repeat;" data-photo="yes"';
        ?>
...
<!-- This section was after header before -->
        ...
        <header class="<?php echo $course_data->post_name; ?> page-cover table 1">
            <div class="table-cell page-header" <?php echo $style; ?>>
                <div class="cover-content">
                    <h1 class="post-title main-title"><?php the_title(); ?>
                    	<small style="font-size: 16px; display: block;"><?php echo $course_name .' <span style="font-size: 23px;">›</span> '. get_the_title(); ?></small>
                    </h1>
                    <div class="table">
...
<?php echo do_shortcode('[course_content course_id="'.get_post_meta( $post->ID, 'course_id', TRUE).'"]'); ?>
