...
<?php else: ?>
    <?php while ( have_posts() ) : the_post(); ?>
        ...
        <?php
        $style = ( has_post_thumbnail() && boss_get_option( 'boss_cover_blog' ) ) ? 'style="background-image: url(' . get_the_post_thumbnail_url( $post , 'extra large' ) . '); background-position: initial;background-size: cover;background-repeat: initial;" data-photo="yes"' : '';
        ?>

<?php if(in_category("crypto-webinars")) { ?>
<style>.page-cover .entry-meta .comments-link a, .page-cover .entry-meta .post-date { color: #434343; text-shadow: none;}</style>
<header class="entry-header <?php if(is_search()){ echo 'page-header'; }?>" style="background-color: transparent; background: #fff; padding: 40px 40px 0 40px !important;">
				...
				<span style=" font-family: 'Raleway', sans-serif; font-size: 26px; line-height: 62px; margin-left: 5px; font-weight: 200; vertical-align: top; color: #363636 !important;">ALLIANCE WEBINARS</span>
</header>
                ...
                <div class="cover-content">
                    ...
                    <div class="table">
                        <div class="table-cell entry-meta">
                            <?php buddyboss_entry_meta($show_author = false, $show_date = true, $show_comment_info = true); ?>
                        </div>
